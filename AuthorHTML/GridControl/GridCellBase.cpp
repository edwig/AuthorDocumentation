...
// implementations of drawing, printing etc provided. MUST be derived
// from to be used.
//
...
CGridCellBase&
CGridCellBase::operator=(const CGridCellBase& cell)
{
	if (this == &cell) 
  {
    return *this;
...
BOOL CGridCellBase::PrintCell(CDC* pDC, int /*nRow*/, int /*nCol*/, CRect rect)
{
  ...
  if (pGrid->GetShadedPrintOut())
  {
    ...
    {
      crBG = (GetBackClr() != CLR_DEFAULT && GetBackClr() != pDefaultCell->GetBackClr()) ? GetBackClr() : CLR_DEFAULT;
    }
