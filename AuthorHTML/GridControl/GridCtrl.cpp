...
//
// PLANS:  1) Implement sparse grids (super easy now)
...
UINT GetMouseScrollLines()
{
  ...
  if(IsWindowsXPOrGreater())
  {
    ...
    if(RegOpenKeyEx(HKEY_CURRENT_USER,_T("Control Panel\\Desktop"),0,KEY_QUERY_VALUE,&hKey) == ERROR_SUCCESS)
    {
      ...
      {
        nScrollLines = (int) _tcstoul(szData,NULL,10);
      }
      RegCloseKey(hKey);
    }
  }
...
// Plans: decrease timer interval over time to speed up selection over time
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
  ...
  if (next != m_idCurrentCell)
  {
    ...
    if (!IsCellVisible(next))
    {
      switch (nChar)
      {
        case VK_RIGHT:SendMessage(WM_HSCROLL, SB_LINERIGHT, 0); 
				              bHorzScrollAction = TRUE;
                      break;
                
        case VK_LEFT: SendMessage(WM_HSCROLL, SB_LINELEFT, 0);  
				              bHorzScrollAction = TRUE;
                      break;
                
        case VK_DOWN: SendMessage(WM_VSCROLL, SB_LINEDOWN, 0);  
				              bVertScrollAction = TRUE;
                      break;
                
        case VK_UP:   SendMessage(WM_VSCROLL, SB_LINEUP, 0);    
				              bVertScrollAction = TRUE;
                      break;                
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
  ...
  switch (pgvItem->lParam)
  {
    ...
    case VK_PRIOR:
    case VK_HOME:
    case VK_END:  OnKeyDown((UINT)pgvItem->lParam, 0, 0);
...
void CGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
  ...
  switch (nSBCode)
  {
    case SB_LINERIGHT:    if (scrollPos < m_nHScrollMax)
                          {
    ...
    case SB_LINELEFT:     if (scrollPos > 0 && idTopLeft.col > GetFixedColumnCount())
                          {
    ...
    case SB_PAGERIGHT:    if (scrollPos < m_nHScrollMax)
                          {
                            rect.left = GetFixedColumnWidth();
    ...
    case SB_PAGELEFT:     if (scrollPos > 0)
                          {
                            rect.left = GetFixedColumnWidth();
                            int offset = -rect.Width();
    ...
    case SB_RIGHT:        if (scrollPos < m_nHScrollMax)
                          {
                            SetScrollPos32(SB_HORZ, m_nHScrollMax);
                            Invalidate();
                          }
...
// Handle vertical scrollbar notifications
void CGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
  ...
  switch (nSBCode)
  {
    case SB_LINEDOWN:     if (scrollPos < m_nVScrollMax)
                          {
    ...
    case SB_LINEUP:       if (scrollPos > 0 && idTopLeft.row > GetFixedRowCount())
                          {
    ...
    case SB_PAGEDOWN:     if (scrollPos < m_nVScrollMax)
                          {
                            rect.top = GetFixedRowHeight();
    ...
    case SB_BOTTOM:       if (scrollPos < m_nVScrollMax)
                          {
...
void CGridCtrl::SetSelectedRange(int nMinRow, int nMinCol,
...
void CGridCtrl::OnSelecting(const CCellID& currentCell)
{
  ...
  switch (m_MouseMode)
  {
    case MOUSE_SELECT_ALL:  SelectAllCells();
                            break;
    case MOUSE_SELECT_COL:  SelectColumns(currentCell, FALSE);
                            break;
    case MOUSE_SELECT_ROW:  SelectRows(currentCell, FALSE);
                            break;
    case MOUSE_SELECT_CELLS:SelectCells(currentCell, FALSE);
                            break;
  }
...
void CGridCtrl::OnUpdateEditPaste(CCmdUI* pCmdUI)
{
  ...
  BOOL bCanPaste = IsValid(cell) && IsCellEditable(cell) && ::IsClipboardFormatAvailable(CF_TEXT);
...
BOOL CGridCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
  if (nHitTest == HTCLIENT)
  {
    switch (m_MouseMode)
    {
      case MOUSE_OVER_COL_DIVIDE: SetCursor(AfxGetApp()->LoadStandardCursor(IDC_SIZEWE));
                                  break;
      case MOUSE_OVER_ROW_DIVIDE: SetCursor(AfxGetApp()->LoadStandardCursor(IDC_SIZENS));
                                  break;
  #ifndef GRIDCONTROL_NO_DRAGDROP
      case MOUSE_DRAGGING:        break;
  #endif
      default:                    if (!GetVirtualMode())
                                  {
                            ...
                            int nColumn  /* = -1 */)
{
  ...
  {
    // Plans: Fix it so column insertion works for in the fixed column area
    ASSERT(FALSE);
    return -1;
  }
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
  ...
  {
    // Plans: Fix it so column insertion works for in the fixed row area
    ASSERT(FALSE);
    return -1;
  }
...
BOOL CGridCtrl::SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass)
{
  ...
  CGridCellBase* pNewCell = dynamic_cast<CGridCellBase*>(pRuntimeClass->CreateObject());
...
/*virtual*/ CGridCellBase* CGridCtrl::CreateCell(int nRow, int nCol)
{
  ...
  CGridCellBase* pCell = dynamic_cast<CGridCellBase*>(m_pRtcDefault->CreateObject());
  if(!pCell)
  {
    return NULL;
  }
...
void
CGridCtrl::InsertItem(int p_row,int p_col,CString& p_text,UINT p_format /* = 0 */)
{
  GV_ITEM item;
...
BOOL CGridCtrl::SetItem(const GV_ITEM* pItem)
{
  ...
  if (pItem->mask & GVIF_TEXT)      pCell->SetText(pItem->strText);
  if (pItem->mask & GVIF_PARAM)     pCell->SetData(pItem->lParam);
  if (pItem->mask & GVIF_IMAGE)     pCell->SetImage(pItem->iImage);
  if (pItem->mask & GVIF_STATE)     SetItemState(pItem->row, pItem->col, pItem->nState);
  if (pItem->mask & GVIF_FORMAT)    pCell->SetFormat(pItem->nFormat);
  if (pItem->mask & GVIF_BKCLR)     pCell->SetBackClr(pItem->crBkClr);
  if (pItem->mask & GVIF_FGCLR)     pCell->SetTextClr(pItem->crFgClr);
  if (pItem->mask & GVIF_FONT)      pCell->SetFont(&(pItem->lfFont));
  if( pItem->mask & GVIF_MARGIN)    pCell->SetMargin( pItem->nMargin);
...
BOOL CGridCtrl::GetItem(GV_ITEM* pItem)
{
  ...
  if (pItem->mask & GVIF_TEXT)      pItem->strText = GetItemText(pItem->row, pItem->col);
  if (pItem->mask & GVIF_PARAM)     pItem->lParam  = pCell->GetData();;
  if (pItem->mask & GVIF_IMAGE)     pItem->iImage  = pCell->GetImage();
  if (pItem->mask & GVIF_STATE)     pItem->nState  = pCell->GetState();
  if (pItem->mask & GVIF_FORMAT)    pItem->nFormat = pCell->GetFormat();
  if (pItem->mask & GVIF_BKCLR)     pItem->crBkClr = pCell->GetBackClr();
  if (pItem->mask & GVIF_FGCLR)     pItem->crFgClr = pCell->GetTextClr();
  if (pItem->mask & GVIF_FONT)      memcpy(&(pItem->lfFont), pCell->GetFont(), sizeof(LOGFONT));
  if( pItem->mask & GVIF_MARGIN)    pItem->nMargin = pCell->GetMargin();
...
UINT CGridCtrl::GetItemState(int nRow, int nCol) const
{
  ...
  if(!pCell)
  {
    // Clicked outside the row/column range
    return 0;
  }
...
BOOL CGridCtrl::AutoSizeRow(int nRow, BOOL bResetScroll /*=TRUE*/)
{
  ...
  //  Skip hidden rows when auto sizing
...
void CGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
  if (bExpandFixed)
	...
	{
    if(GetColumnCount() <= GetFixedColumnCount())
    {
      return;
    }
	}
  ...
  if (nDifference > 0)
  {
    ...
    for (int nCount = 0, col = nFirstColumn; (col < GetColumnCount()) && (nCount < leftOver); col++, nCount++)
    {
      if(m_arColWidths[col] > 0)
      {
        m_arColWidths[col] += 1;
...
void CGridCtrl::ExpandRowsToFit(BOOL bExpandFixed /*=TRUE*/)
{
  ...
  if (nDifference > 0)
  {
    ...
    for (int nCount = 0, row = nFirstRow;  (row < GetRowCount()) && (nCount < leftOver); row++, nCount++)
    {
      if(m_arRowHeights[row] > 0)
      {
...
void CGridCtrl::OnMouseMove(UINT /*nFlags*/,CPoint point)
{
  ...
  if (m_bLMouseButtonDown)
  {
    switch (m_MouseMode)
    {
      ...
      case MOUSE_PREPARE_DRAG:  m_MouseMode = MOUSE_PREPARE_DRAG;
                                OnBeginDrag();    
                                break;
#endif
    }    
...
#endif
  {
    ...
    if (m_LeftClickDownCell.row < GetFixedRowCount())
    {
      ...
      if(m_AllowReorderColumn && m_LeftClickDownCell.col >=  GetFixedColumnCount())
	    {
		    ResetSelectedRange(); 
        // This is not the better solution, as we do not see why clicking in column header should reset selection
...
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
  ...
  {
    SetFocusCell(-1,-1);
    SetFocusCell(__max(FocusCell.row, m_nFixedRows),__max(FocusCell.col, m_nFixedCols));
...
void CGridCtrl::SetPrintMarginInfo(int nHeaderHeight, int nFooterHeight,
                                   ...
                                   int nTopMargin,    int nBottomMargin,
                                   int nGap)
{
...
void CGridCtrl::OnPrint(CDC *pDC, CPrintInfo *pInfo)
{
  ...
  while(m_nCurrPrintRow < GetRowCount() && (int)nNumPages < iPageIfIgnoredWideCols)
  {
