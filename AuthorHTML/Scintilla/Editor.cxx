...
public:
	...
	void SetAt(int p_line) 
  {
		if (p_line < lineStart || p_line > lineEnd) 
    {
    ...
    else 
    {
			if (ed->selType == ed->selRectangle) 
      {
				...
				startPos = ed->PositionFromLineX(p_line, minX);
				// Measure line and return character closest to maxX
				endPos = ed->PositionFromLineX(p_line, maxX);
			} 
      else if (ed->selType == ed->selLines) 
      {
				startPos = ed->pdoc->LineStart(p_line);
				endPos   = ed->pdoc->LineStart(p_line + 1);
			} 
      else 
      {	
        // Stream selection, here only for completion
				if (p_line == lineStart) 
        {
					startPos = selStart;
				} 
...
void Editor::PasteRectangular(int pos, const char *ptr, int len) {
	...
	for (int i = 0; i < len; i++) {
      ...
      {
				for (int ind = 0; ind < xInsert - XFromPosition(currentPos); ind++) 
        {
