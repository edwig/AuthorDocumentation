...
Copyright (c) 2004 - 2006 by PJ Naughter (Web: www.naughter.com, Email: pjna@naughter.com)
...
{
  //Allocate some heap memory to contain the text
  TCHAR* pszSelText = new TCHAR[GetSelectionEnd() - GetSelectionStart() + 1];

  //Call the function which does the work
  GetSelText(pszSelText);

  //Create the string for the return value and clean up the heap memory before we exit
...
{
  Call(SCI_SETINDENTATIONGUIDES, static_cast<WPARAM>(show), 0, bDirect);
}
