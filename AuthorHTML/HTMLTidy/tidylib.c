...
int tidyDocCleanAndRepair( TidyDocImpl* doc )
{
    ...
    if ( doc->root.content )
    {
        ...
        {
            Node* node2 = TY_(FindDocType)(doc);
            /* Remove reference, but do not free */
            if (node2)
              TY_(RemoveNode)(node2);
        }
...
void TIDY_CALL tidyAttrSetValue(TidyDoc tdoc, TidyAttr tattr,tmbstr p_value)
{
  AttVal* attval   = tidyAttrToImpl(tattr);
  TidyDocImpl* doc = tidyDocToImpl(tdoc);
  if(attval)
  {
    ...
    attval->value = TY_(tmbstrdup)(doc->allocator, p_value);
  }
}
